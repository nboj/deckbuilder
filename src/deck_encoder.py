...
class DeckEncoderDecoder: 
    ...
    def encode(self):
        ...
        deck_final = ""
        for item in normal:
            deck_final+=str(item[0])+self._DELIMITER+str(item[1])+','
        if len(normal)==0:
            deck_final+='N'
        else:
    ...
    def decode(self):
        ...
        final_split = deck_final.split(';')
        normal = []
        tc = []
        items = []
        if final_split[0]!="N":
            normal = final_split[0].split(',')
        if final_split[1]!="N":
            tc = final_split[1].split(',')
        if final_split[2]!="N":
            items = final_split[2].split(',')

        new_deck['normal'] = {}
        new_deck['tc'] = {}
        new_deck['item'] = {}
        for item in normal:
            sub_items = item.split(self._DELIMITER)
